...
const createPost = async (req, res) => {
  try {
    const { text, vacationId } = req.body;
    ...
    if (!text && images.length === 0) {
      return res
        .status(400)
        .json({ error: 'Post must have either text or images' });
    }

    const newPost = new Post({
      user: userId,
      text,
      images, // Lưu array images thay vì img
    });

    await newPost.save();

    // Thêm bài viết vào kỳ nghỉ nếu có
    if (vacationId) {
    ...
    // Populate user info
    await newPost.populate('user', 'username fullName profileImg');
